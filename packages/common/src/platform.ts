...
if (typeof globals.docage !== 'undefined' && typeof globals.docage.process !== 'undefined') {
	// Native environment (sandboxed)
	nodeProcess = globals.docage.process;
...
// Native environment
	...
	const rawNlsConfig = nodeProcess.env['DOCAGE_NLS_CONFIG'];
	if (rawNlsConfig) {
		try {
			...
			// Docage's default language is 'zh-CN'
...
export const setTimeout0 = (() => {
	if (setTimeout0IsFaster) {
		...
		globals.addEventListener('message', (e: MessageEvent) => {
			if (e.data && e.data.docageScheduleAsyncWork) {
				for (let i = 0, len = pending.length; i < len; i++) {
					const candidate = pending[i];
					if (candidate.id === e.data.docageScheduleAsyncWork) {
						pending.splice(i, 1);
						candidate.callback();
						return;
					}
		...
		return (callback: () => void) => {
			...
			globals.postMessage({ docageScheduleAsyncWork: myId }, '*');
		};
	}
